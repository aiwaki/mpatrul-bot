...
import { fetchClassification, fetchPages, PageState, updatePageState, type Classification, type Page } from '../../database/pages';
...
const _fetchPagesCron = new Cron('1 * * * *', async () => {
    pages = await getPages();
    console.log('Страницы успешно обновлены.');
});
...
async function sendPageCard(ctx: Scenes.WizardContext, page: { info: Page; classification: Classification }) {
    const message = fmt`
